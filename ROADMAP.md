...
- [x] 6.4 `src/autoload/settings.gd`: `auto_quality_tick` حالا نمونه‌ها را در یک پنجرهٔ متحرک ۵تایی (`AQ_WINDOW=5`) میانگین می‌گیرد قبل از مقایسه با آستانهٔ ۴۰fps، پس یک افت لحظه‌ای (GC، باز شدن منو) به‌تنهایی کیفیت را پایین نمی‌آورد؛ تست‌های `tests/verify.gd` هم برای این رفتار جدید بازنویسی شدند
- [x] 6.5 `src/entities/projectile.gd`: با ساخت یک دیمن واقعی داخل یک `Dungeon` زنده در `tests/verify.gd` و شلیک واقعی fireball تأیید شد که منطق فعلی parent-chain (`ball → actors → Dungeon`) درست resolve می‌شود چون `Dungeon` خودش `is_walkable_at` را پیاده‌سازی می‌کند (کد نیازی به تغییر نداشت، فقط پوشش تست کم بود؛ نگرانی روادمپ دربارهٔ Interior هم بی‌مورد بود چون هیچ دشمنی داخل Interior اسپان نمی‌شود)
- [x] 6.6 `src/ui/touch_ui.gd`: بررسی دقیق کد نشان داد این نگرانی از اول بی‌مورد بوده — `_to_world` مستقیماً `hero.get_canvas_transform()` را صدا می‌زند (یک متد پایهٔ `CanvasItem` که همیشه مقدار برمی‌گرداند، حتی اگر `hero.cam` هیچ‌وقت `make_current()` نشده باشد) نه چیزی روی خودِ آبجکت `Camera2D`؛ تنها حالت null واقعی (وقتی خودِ `hero` نال است) از قبل هندل شده بود. `_check_touch_quality()` هم این مسیر را در سناریوی واقعی (تپ→حرکت) پوشش می‌دهد و پاس است — نیازی به تغییر کد نبود.
...
- [x] 6.9 `src/entities/bed.gd`, `stairs.gd`: هر دو تأیید شدند که واقعاً هر فریم `I18N.tr_str`+`I18N.tag` را دوباره اجرا می‌کردند؛ حالا فقط یک‌بار در `_ready()` و بعد فقط با سیگنال `I18N.locale_changed` رفرش می‌شوند؛ برای stairs یک متد `set_locked()` جدید اضافه شد تا وقتی باس دانجن می‌میرد (`dungeon.gd _on_boss_down`) پرامپت واقعاً از "مهروموم" به "تپ کن" عوض شود بدون نیاز به چک هر فریم
...
- [x] P2-fix2.3 **(لایهٔ دوم مشکل، بعد از یک اجرای دیگر روی سرور کشف شد):** با وجود این‌که `android/build/` حالا واقعاً پر می‌شد (تأیید شد در لاگ CI با نام قدم موفق)، Export باز هم شکست خورد، این‌بار با پیام دقیق‌تر: `ERROR: Export: Trying to build from a gradle built template, but no version info for it exists. Please reinstall from the 'Project' menu.` — با خواندن مستقیم سورس Godot (`editor/export/export_template_manager.cpp` تابع `install_android_template_from_file`) مشخص شد ادیتور علاوه بر `android/build/`، یک فایل کنترل نسخه به اسم `android/.build_version` هم می‌نویسد (حاوی رشتهٔ نسخهٔ گودو، مثلاً `4.4.1.stable`) و در اکسپورت این رشته را با نسخهٔ فعلی مقایسه می‌کند؛ اگر فایل نبود یا نسخه نخورد، اکسپورت را رد می‌کند. قدم «Install Android Gradle build template» را کامل کردم تا این فایل را هم بنویسد (از نام دایرکتوری export-templates مشتق می‌شود، نه هاردکد، تا اگر نسخهٔ گودو در ورک‌فلو عوض شد خودش هماهنگ بماند).

### ✅ نتیجهٔ نهایی تأییدشده روی سرور واقعی GitHub Actions (نه فقط لوکال)
کامیت `fc5187a` را پوش کردم و صبر کردم هر دو ورک‌فلو کامل تمام شوند:
- **`CI` (run 34059030014): success** — یعنی 568/568 روی هدلس اجرای واقعی CI.
- **`android-apk` (run 34059030010): success**، تک‌تک قدم‌ها سبز از جمله «Sanity — signature, alignment, universal ABIs, manifest»، و یک APK امضاشدهٔ ۵۵٫۷ مگابایتی (`pixel-realms-universal-apk`) به‌عنوان artifact آپلود شد.

این اولین باری در کل این نشست است که هر دو گیت CI روی سرور واقعی (نه ادعا، نه فقط لوکال) کاملاً سبز شده‌اند. تمام ۱۴ فیل اصلی + رگرسیون‌های خودساختهٔ فاز P2 + هر دو مشکل زیرساخت اکسپورت اندروید برطرف شدند.
